...
export default function Home() {
    ...
    const performEvaluation = async (question: Question, currentIndex: number) => {
        ...
        if (question.keyDiagram) {
            ...
            console.log("📦 FormData - diagram_0:", question.keyDiagram.name, question.keyDiagram.size);
        }
        
        if (question.studentSheet) {
            formData.append("answer_sheets", question.studentSheet);
            console.log("📦 FormData - answer_sheets:", question.studentSheet.name, question.studentSheet.size);
        }
        ...
        try {
            ...
            setQuestions(prevQuestions => {
                ...
                updated[currentIndex].result = {
                    obtained: result.totalObtained,
                    textSimilarity: result.breakdown[0]?.textSimilarity || 0,
                    diagramSimilarity: result.breakdown[0]?.diagramSimilarity || 0
                };
                return updated;
            });
    ...
    const evaluateAllQuestions = async () => {
        ...
        for (let i = 0; i < currentQuestions.length; i++) {
            const q = currentQuestions[i];
            console.log(`📝 Evaluating question ${i + 1}/${currentQuestions.length}...`);
                                ...
                                >
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {isIncomplete && !isCompleted && (
                                            <span className="text-xs px-2 py-1 bg-yellow-100 text-yellow-700 rounded-full">!</span>
                                        )}
                                    </div>
                                </button>
                            );
                    ...
                    {evaluationResult && (
                        <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: '#0F2854' }}>
                            ...
                            <p className="text-2xl font-light text-white">
                                {evaluationResult.totalObtained}/{evaluationResult.totalPossible}
                            </p>
                    ...
                    <div className="flex flex-col gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-6 p-3 bg-gray-50 rounded-lg">
                            ...
                            {!currentQuestion.keyDiagram && (
                                <span className="text-xs text-gray-400 italic">Add diagram to enable weight adjustment</span>
                            )}
                        </div>
                    </div>
                                    ...
                                    {!currentQuestion.studentSheet ? (
                                        <>
                                            <input
                                                ...
                                                className="hidden"
                                                onChange={(e) => handleStudentSheetUpload(currentPage, e.target.files?.[0] || null)}
                                            />
                                            <label
                                        ...
                                        <div className="w-full text-center">
                                            ...
                                            <button
                                                onClick={() => handleStudentSheetUpload(currentPage, null)}
                                ...
                                {currentQuestion.result && (
                                    <div className="p-4 rounded-lg" style={{ backgroundColor: '#BDE8F5' }}>
                                        ...
                                        <div className="flex justify-between text-xs mt-2 text-gray-600">
                                            <span>
                                                Text: {(currentQuestion.result.textSimilarity * 100).toFixed(0)}% 
                                                <span className="ml-1 text-gray-400">
                                                    (weight: {(currentQuestion.textWeight * 100).toFixed(0)}%)
                                                </span>
                                            </span>
                                            <span>
                                                Diagram: {(currentQuestion.result.diagramSimilarity * 100).toFixed(0)}%
                                                <span className="ml-1 text-gray-400">
                                                    (weight: {(currentQuestion.diagramWeight * 100).toFixed(0)}%)
                                                </span>
                                            </span>
                                        </div>
                                        <div className="text-xs text-right mt-1 text-gray-500">
                                            Weighted avg: {(
                                                (currentQuestion.result.textSimilarity * currentQuestion.textWeight) +
                                                (currentQuestion.result.diagramSimilarity * currentQuestion.diagramWeight)
                                            ).toFixed(2)}
                                        </div>
                                    </div>
                                )}
