...
<div className="mt-4 flex">
            ...
            {imageName}</label>
                    ...
                    <div className="flex w-full mt-3 gap-4">
                        <div className="flex flex-col w-1/2">
                            <label className="font-bold text-md">Text Weight (0 - 1)</label>
                            <input
                                type="number"
                                value={textWeight}
                                min={0}
                                ...
                                className="bg-dy-peach rounded-xl px-3 py-1 text-center text-dy-red font-bold"
                            />
                        </div>
                        <div className="flex flex-col w-1/2">
                            <label className="font-bold text-md">Diagram Weight (0 - 1)</label>
                            <input
                                type="number"
                                value={diagramWeight}
                                min={0}
                                max={1}
                                step={0.1}
                                onChange={(e) => setDiagramWeight(Number(e.target.value))}
                                className="bg-dy-peach rounded-xl px-3 py-1 text-center text-dy-red font-bold"
                            />
                        </div>
                    </div>
    ...
    <div className="grid grid-cols-4 grid-row-5 gap-6 w-full px-10 mt-5">
        {fileNames.map((name, index) => {
            ...
            const totalScore = (
                textSimilarity * textWeight + diagramSimilarity * diagramWeight
            ) * marks;
                ...
                >
                    ...
                    {similarityData[index] && (
                        <div className="mt-2 flex flex-col items-center">
                            ...
                            <p className="mt-2 font-semibold text-dy-red">Total Score: {totalScore.toFixed(1)}/{marks.toString()}</p>
                        </div>
                    )}
                </button>
            );
...
{/* Expanded View */}
{openedSheet !== null && (
    <div className="w-full h-full flex flex-col items-center justify-start bg-white rounded-3xl p-6 shadow-lg overflow-y-auto max-h-full">
